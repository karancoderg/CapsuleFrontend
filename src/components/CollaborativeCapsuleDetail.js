...
const CollaborativeCapsuleDetail = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await api.post(`/api/capsules/${capsuleId}/entries`, payload, {
        headers: {
          ...
          return (
            <li key={entry._id} className="entry-node">
              ...
              <div className="entry-footer">
                <small>Added by: {entry.createdBy?.name || entry.createdBy || "Unknown Member"}</small>
              </div>
            </li>
          )
        })}
  ...
  return (
    <div className="collab-capsule-detail">
      ...
      {capsule && !capsule.lockDate && <p className="capsule-description">{capsule.description}</p>}
      ...
      <form onSubmit={handleSubmit} className="entry-form">
        <textarea
          ...
          required
        ></textarea>
        <input type="file" name="entryFile" onChange={handleFileChange} accept="image/*,video/*,audio/*" />
        <input
          type="date"
          name="lockDate"
